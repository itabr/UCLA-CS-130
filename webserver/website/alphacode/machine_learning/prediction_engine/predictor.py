...
class VSMSampleMapper(object):
	def __init__(self, prefix = "", idf_file_type = 'pickle'):
		if idf_file_type == "pickle":
			f = open("alphacode/machine_learning/" + prefix + "idf.txt")
...
class RidgeCLF(object):
	def __init__(self, prefix = ""):
		f = open("alphacode/machine_learning/" + prefix + "ridge_clf.txt")
		self.clf = pickle.loads(f.read())
		f.close()
		f = open("alphacode/machine_learning/" + prefix + "binarizer.txt")
